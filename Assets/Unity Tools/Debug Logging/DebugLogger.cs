...
// Comment out #defines to choose debug level and whether to display logs in game
//#define DEBUG_LEVEL_WARN
#define DEBUG_LEVEL_ERROR
//#define DEBUG_LEVEL_CRITICAL
//#define DEBUG_LEVEL_INFO
#define DRAW_LOGS_TO_SCREEN
...
public class DebugLogger : MonoBehaviour {
	...
	private void OnGUI () {
		...
		float Multiplier = Screen.height / 1080.0f;
		Rect drawOffset = new Rect ( 25 , 600 * Multiplier , 200 , 50 );
		...
		if ( m_currentLogs != null ) {
			...
			for ( int i = numEntries - 1 ; i >= 0 ; --i ) {
				...
				drawOffset.y -= 50 * Multiplier;

				if ( drawOffset.y < 100 * Multiplier ) {
					break;
				}
				else {
					numDrawn++;
